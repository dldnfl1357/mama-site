...
1. **이 repo는 frontend 전용.** 이 디렉토리에서 작업할 때는 backend(별도 외부 시스템)의 코드·스키마·다음-할-일을 작성하거나 계획하지 않는다. backend는 `/api/*`를 제공하는 외부 의존성으로만 참조한다. API 계약 불일치·서버 다운 등이 보이면 frontend 측에서 표시·우회만 처리하고, backend 변경이 필요한 경우 사용자에게 알린 뒤 frontend 작업은 거기서 멈춘다. WORKLOG·TODO에 backend 작업 항목을 넣지 않는다.
2. **프로덕션 DB 직접 쿼리 금지.** 프론트엔드 코드(브라우저 런타임)는 절대 DB에 직접 붙지 않는다. 모든 데이터는 `/api/...` 엔드포인트를 통해 받는다. SQL·DB 클라이언트 의존성을 추가하는 PR은 거부한다.
3. **시크릿 파일 커밋 금지.** `.env`, `.env.local`, `*.pem`, API 키·토큰·세션 쿠키 값이 들어간 어떤 파일도 절대 `git add` 하지 않는다. `.gitignore`에 누락된 시크릿 파일을 발견하면 즉시 알린다. 토큰을 채팅·코드·커밋 메시지에 평문으로 남기지 않는다.
...
- `types.ts`는 `/api/*` 응답 구조의 1:1 거울. 화면 전용 파생 타입은 해당 페이지 안에서 정의.
...
- API 서버가 떠 있지 않으면 fetch는 5xx로 떨어지고 각 페이지의 에러 패널이 표시된다. 이는 정상 동작이며, API 서버 미동작 자체를 프론트 버그로 보고하지 않는다(서버 측 수정은 이 repo 범위 밖).
...
`mama-site`는 **개인용 한국 주식 자동매매 시스템의 모니터링 UI**다. 매매 실행과 KIS API 호출은 외부 API 서버가 담당하고, 본 프론트엔드는 그 서버가 노출하는 `/api/*`를 소비하는 **읽기 전용 콘솔**이다.

```
[외부: KIS · DART · 매매 엔진]    ← 이 repo 범위 밖
...
1. **API 서버는 기본 모의투자 모드를 가정.** 응답의 `paperTrading` 플래그가 true면 실제 잔고가 아닐 수 있다. 프론트에서 이 플래그를 chip으로 항상 보이게 표시한다.
2. **숫자는 KRW 정수.** 소수점 표시·반올림 처리를 임의로 바꾸지 않는다. 모든 통화 표시는 `format.ts`의 `formatKrw` / `formatSignedKrw`를 사용.
3. **잔고·매매일지는 시점 데이터.** "최신" 자동 폴링을 도입하기 전, 새로고침 비용(외부 API 호출량)을 먼저 검토.
